...
## 디렉티브(Directive)
디렉티브란 Vue에서 제공하는 특수 속성이다. **v-** 접두어가 붙어있으며 랜더링 된 DOM에 반응하는 동작을 실행해주는 역할을 한다.

1. v-text : 텍스트 바인딩을 위한 속성. {{ }}과 동일한 기능을 가짐
2. v-html : v-text는 xxxx에 반해, 태그 자체가 텍스트형태로 랜더링 됨(템플릿 구문도 마찬가지)
3. v-show : 해당  true or false 값에 따라 display로 노출여부가 결정 된다
4. v-if : 조건값이 true or false에 다라 랜더링 여부가 결정된다
5. v-else
6. v-else-if
7. v-bind : 태그 속성을 바인딩을 위한 속성
8. v-model : 양방향 바인딩
9. v-on
10. v-cloak
11. v-once
